...
const buildFollowUpMatch = async (req, { kind, leadId }) => {
  ...
  if (req.user.role === 'sales') {
    const ownedLeadIds = await Lead.find({ assignedSales: req.user._id }).distinct('_id')
    if (kind === 'manual') {
      match.$or = [
...
exports.getFollowUpActivity = asyncHandler(async (req, res) => {
  const kind = String(req.query.kind || 'manual').trim().toLowerCase()
  const view = String(req.query.view || 'summary').trim().toLowerCase()
  if (!VALID_KINDS.includes(kind)) return badRequest(res, 'Invalid kind. Use: manual, automatic')
  if (!VALID_VIEWS.includes(view)) return badRequest(res, 'Invalid view. Use: summary, detail')
  ...
  const allRows = await FollowUp.find(match).select('leadId followUpDate status').lean()
  ...
  for (const row of allRows) {
    ...
    const lastTime = bucket.lastFollowUpAt ? new Date(bucket.lastFollowUpAt).getTime() : 0
