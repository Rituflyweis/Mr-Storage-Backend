...
const computeProjectSteps = (lead, stepDetails = []) => {
  ...
  const foundIdx = PROJECT_STEP_DEFS.findIndex((b) =>
    b.stages.includes(currentStage),
  );
  ...
  const steps = PROJECT_STEP_DEFS.map((bucket, idx) => {
    const entries = history
      .filter((h) => bucket.stages.includes(h.stage))
    ...
    return {
      ...
      completedAt:
        detail?.completedAt || (status === "completed" ? date : null),
...
exports.getProfile = asyncHandler(async (req, res) => {
  ...
  const profile = shapeCustomerProfile(customer);
  return success(res, { profile, customer: profile });
});

exports.updateProfile = asyncHandler(async (req, res) => {
  const { name, firstName, lastName, email, phone, countryCode, mobile, photo } =
    req.body;
  ...
  if (!hasUpdate) {
    return badRequest(
      res,
      "No updatable fields provided — send name, email, phone, mobile, or photo",
    );
  }
  ...
  const nameError = applyCustomerNameInput(customer, { name, firstName, lastName });
  ...
  const parsedPhone = parseCustomerPhoneInput(phone, countryCode);
  if (parsedPhone !== undefined) {
    if (!parsedPhone.number) return badRequest(res, "Phone number is required");
    customer.phone = parsedPhone;
  }

  if (mobile !== undefined) customer.mobile = parseOptionalTrimmedString(mobile);
...
exports.getDashboard = asyncHandler(async (req, res) => {
  ...
  const [leads, invoices] = await Promise.all([
    Lead.find({ customerId })
      .select(
        "lifecycleStatus quoteValue documents jobId projectName location buildingType assignedSales updatedAt",
      )
      .lean(),
    Invoice.find({ customerId })
      .select("status totalAmount date daysToPay leadId")
      .lean(),
  ]);

  const activeProjects = leads.filter(
    (l) => !CLOSED_STAGES.includes(l.lifecycleStatus),
  ).length;
  const closedProjects = leads.filter((l) =>
    CLOSED_STAGES.includes(l.lifecycleStatus),
  ).length;

  const drawingsAndApprovals = leads.reduce(
    (sum, l) =>
      sum +
      l.documents.filter((d) => d.type === "drawing" || d.type === "approval")
        .length,
    0,
  );

  const totalProjectValue = leads.reduce(
    (sum, l) => sum + (l.quoteValue || 0),
    0,
  );
  ...
  if (upcomingRaw) {
    const lead = leads.find(
      (l) => String(l._id) === String(upcomingRaw.leadId),
    );
    upcomingInvoice = {
  ...
  const deliveries = leadIds.length
    ? await Delivery.find({
        ...
        .select(
          "status deliveryDate deliveryNumber description loadDescription leadId loadWeight",
        )
        .lean()
    : [];
  ...
  const deliveryTracking = {
    inTransit: deliveries.filter((d) =>
      CUSTOMER_IN_TRANSIT_STATUSES.has(d.status),
    ).length,
    staged: deliveries.filter((d) => d.status === "scheduled").length,
    ready: deliveries.filter((d) => d.status === "confirmed").length,
    totalToday: deliveries.filter(
      (d) =>
        d.deliveryDate &&
        new Date(d.deliveryDate).toDateString() === nowTs.toDateString(),
    ).length,
    upcomingDeliveries: deliveries.filter(
      (d) => d.deliveryDate && new Date(d.deliveryDate) > nowTs,
    ).length,
    deliveriesThisWeek: deliveries.filter(
      (d) =>
        d.deliveryDate &&
        new Date(d.deliveryDate) > nowTs &&
        new Date(d.deliveryDate) <= weekFromNow,
    ).length,
    delayedDeliveries: deliveries.filter((d) => d.status === "delayed").length,
    rescheduledDeliveries: deliveries.filter((d) => d.status === "rescheduled")
      .length,
  };

  const nextDeliveryRaw =
    deliveries
      .filter(
        (d) =>
          d.deliveryDate &&
  ...
  if (nextDeliveryRaw) {
    ...
    const nextDeliveryLead = leads.find(
      (l) => String(l._id) === String(fullDelivery.leadId),
    );
    ...
    nextDelivery = mapDeliveryRow(
      fullDelivery,
      nextDeliveryLead || { _id: fullDelivery.leadId },
      carrier,
      loadDetails,
    );
  }

  const totalBundles = leadIds.length
    ? await Bundle.countDocuments({ leadId: { $in: leadIds } })
    : 0;
  ...
  const projectTimeline = nextMilestone
    ? Math.max(
        0,
        Math.ceil((new Date(nextMilestone.targetDate) - nowTs) / 86400000),
      )
    : null;
  ...
  if (activeLead) {
    const [currentStep, leadNextMilestone, projectManager] = await Promise.all([
      ProjectStepDetail.findOne({ leadId: activeLead._id })
        .sort({ updatedAt: -1 })
        .lean(),
      Milestone.findOne({
        ...
        .sort({ targetDate: 1 })
        .lean(),
      activeLead.assignedSales
        ? User.findById(activeLead.assignedSales).select("name phone").lean()
        : null,
    ]);
    ...
    activeProjectOverview = {
      ...
      nextMilestone: leadNextMilestone
        ? {
            title: leadNextMilestone.title,
            targetDate: leadNextMilestone.targetDate,
          }
        : null,
      projectManager: projectManager
        ? { name: projectManager.name, phone: projectManager.phone || "" }
        : null,
  ...
  const drawingStatusAgg = leadIds.length
    ...
    : [];
  const drawingStatusMap = Object.fromEntries(
    drawingStatusAgg.map((s) => [s._id, s.count]),
  );
  const revisionReceivedCount = leadIds.length
    ? await DrawingDocument.countDocuments({
        leadId: { $in: leadIds },
        revisionRequestedAt: { $ne: null },
      })
    : 0;
  const latestPendingDrawing = leadIds.length
    ? await DrawingDocument.findOne({
        leadId: { $in: leadIds },
  ...
  const drawingsApprovalsBreakdown = {
    pendingReview:
      (drawingStatusMap.pending || 0) + (drawingStatusMap.under_review || 0),
    pendingReviewSubtitle:
      latestPendingDrawing?.buildingLabel ||
      latestPendingDrawing?.documentType ||
      "",
  ...
  const dashboardQuotations = rawOrders.length
    ? await OrderQuotation.find({
        orderId: { $in: rawOrders.map((o) => o._id) },
      })
        .sort({ createdAt: -1 })
        .lean()
    : [];
  ...
  for (const q of dashboardQuotations) {
    ...
    if (!latestQuotationByOrderForDashboard.has(key))
      latestQuotationByOrderForDashboard.set(key, q);
  }
  const ordersList = rawOrders.map((order) => {
    const lead =
      leads.find((l) => String(l._id) === String(order.leadId)) || {};
    const totalQuantity = order.requestedItems.reduce(
      (sum, i) => sum + (i.quantity || 0),
      0,
    );
    const totalLength = order.requestedItems.reduce(
      (sum, i) => sum + (i.quantity || 0) * (i.lengthFeet || 0),
      0,
    );
    return {
      ...order,
      project: {
        leadId: lead._id || order.leadId,
        projectName: lead.projectName || "",
        jobId: lead.jobId || "",
      },
      totalQuantity,
      totalLength,
      stage: computeOrderStage(
        order,
        latestQuotationByOrderForDashboard.get(String(order._id)),
      ),
    };
  });
...
exports.getProjects = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    Lead.find(filter)
      .select(
        "jobId projectName buildingType location lifecycleStatus quoteValue isQuoteReady isRaisedToPO source documents assignedSales plannedStartDate endDate",
      )
    ...
    Lead.countDocuments({
      ...baseFilter,
      lifecycleStatus: { $in: ACTIVE_STAGES },
    }),
    Lead.countDocuments({
      ...baseFilter,
      lifecycleStatus: { $in: WIP_STAGES },
    }),
  ...
  const stepDetails = projectIds.length
    ? await ProjectStepDetail.find({ leadId: { $in: projectIds } })
        .sort({ updatedAt: -1 })
        .select("leadId completionPct")
        .lean()
    : [];
  const progressByLead = new Map();
  for (const s of stepDetails) {
    const key = String(s.leadId);
    if (!progressByLead.has(key))
      progressByLead.set(key, s.completionPct ?? null);
  }

  const projectsWithProgress = projects.map((p) => ({
    ...enrichLeadDocument(p),
    progressPct: progressByLead.get(String(p._id)) ?? null,
    manager: p.assignedSales
      ? { name: p.assignedSales.name, email: p.assignedSales.email }
      : null,
  }));
...
exports.getProject = asyncHandler(async (req, res) => {
  ...
  const formattedPaymentSchedule =
    formatCustomerPaymentSchedule(paymentScheduleDoc);
  ...
  const next =
    idx !== -1 && idx < siblings.length - 1 ? siblings[idx + 1] : null;
  ...
  return success(res, {
    ...
    navigation: {
      previous: prev
        ? { _id: prev._id, jobId: prev.jobId, projectName: prev.projectName }
        : null,
      next: next
        ? { _id: next._id, jobId: next.jobId, projectName: next.projectName }
        : null,
    },
  });
});
...
exports.getDocuments = asyncHandler(async (req, res) => {
  ...
  const [drawingDocs, plantBuildings] = await Promise.all([
    ...
    Building.find({ leadId: { $in: leadIds } })
      .select("leadId buildingNumber drawings uploadedBy")
      .lean(),
  ]);
  ...
  for (const lead of leads) {
    ...
    let embeddedDocs = (lead.documents || []).map((d) => ({
      ...d,
      source: "lead",
    }));
...
exports.getPaymentStats = asyncHandler(async (req, res) => {
  ...
  const invoices = await Invoice.find({ customerId })
    .select("status totalAmount dueDate createdAt")
    .lean();
  ...
  const upcomingCount = invoices.filter(
...
exports.getPresignedUrl = asyncHandler(async (req, res) => {
  ...
  if (!fileName || !fileType)
    return badRequest(res, "fileName and fileType are required");
...
exports.getPaymentInvoices = asyncHandler(async (req, res) => {
  ...
  if (tab === "paid") invoiceFilter.status = "paid";
    ...
    invoiceFilter.status = { $in: ["sent", "overdue"] };
  else if (status) invoiceFilter.status = status;
  if (leadId) invoiceFilter.leadId = leadId;
  if (search?.trim())
    invoiceFilter.invoiceNumber = { $regex: search.trim(), $options: "i" };
  ...
  const [invoices, leads] = await Promise.all([
    ...
    Lead.find({ customerId })
      .select(
        "projectName jobId buildingType location lifecycleStatus quoteValue",
      )
      .lean(),
  ]);
  ...
  const projects = Object.entries(grouped).map(([leadId, invs]) => {
    ...
    const projectPaid = invs
      .filter((i) => i.status === "paid")
      .reduce((s, i) => s + (i.totalAmount || 0), 0);
    const projectPending = invs
    ...
    return {
      ...
      summary: {
        ...
        overdueAmount,
        paidPercent:
          projectTotal > 0 ? Math.round((projectPaid / projectTotal) * 100) : 0,
...
exports.getPaymentInvoiceDetail = asyncHandler(async (req, res) => {
  ...
  if (String(invoice.customerId) !== String(req.customer._id))
    return forbidden(res, "This invoice does not belong to your account");

  const [lead, paymentSchedule] = await Promise.all([
    Lead.findById(invoice.leadId)
      .select("projectName jobId buildingType location")
      .lean(),
...
exports.submitPaymentProof = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(files) || !files.length)
    return badRequest(res, "At least one receipt file is required");
...
exports.getInvoiceStats = asyncHandler(async (req, res) => {
  ...
  const [invoices, leads] = await Promise.all([
    Invoice.find({ customerId })
      .select("status totalAmount dueDate date leadId")
      .lean(),
...
const buildPackingListSummary = (delivery, loadDetails) => {
  ...
  const bundleTypes = new Set(bundles.map((b) => b.bundleType).filter(Boolean))
    .size;
...
const mapDeliveryRow = (d, lead, carrier, loadDetails) => {
  ...
  const latest = rescheduleHistory.length
  ...
  return {
    ...
    deliveryCompany,
    deliveryTeam: deliveryCompany
      ? {
          company: deliveryCompany.name,
          driver: deliveryCompany.driver,
...
const assertDeliveryOwner = async (req) => {
  ...
  const lead = await Lead.findById(delivery.leadId)
    .select("customerId jobId projectName")
    .lean();
  if (!lead || String(lead.customerId) !== String(req.customer._id))
    return null;
...
const buildDeliveryScheduleResponse = async (leads, tab) => {
  ...
  const selectedBidIds = deliveries
    .map((d) => d.selectedCarrierBidId)
    .filter(Boolean);
  ...
  const loadDetailsEntries = await Promise.all(
    uniqueLeadIds.map(async (id) => [
      id,
      await loadFreightLoadDetailsByLeadId(id),
    ]),
  );
  ...
  const result = deliveries.map((d) => {
    ...
    const bid = d.selectedCarrierBidId
      ? bidMap.get(String(d.selectedCarrierBidId))
      : null;
  ...
  const pastCount = await Delivery.countDocuments({
    leadId: { $in: leadIds },
    $or: [
      { status: "delivered" },
      { deliveryDate: { $lt: now }, status: { $nin: ["draft", "cancelled"] } },
  ...
  const rescheduledCount = await Delivery.countDocuments({
    leadId: { $in: leadIds },
    status: "rescheduled",
  });
...
exports.getDeliverySchedule = asyncHandler(async (req, res) => {
  ...
  const leads = await Lead.find(leadFilter)
    .select("_id jobId projectName")
    .lean();
  if (!leads.length)
    return success(res, {
      deliveries: [],
      total: 0,
      tabs: { upcoming: 0, past: 0, rescheduled: 0 },
    });
...
exports.getDeliveryScheduleOrDetail = asyncHandler(async (req, res, next) => {
  ...
  let lead = null;
  try {
    lead = await Lead.findOne({ _id: id, customerId: req.customer._id })
      .select("_id jobId projectName location")
      .lean();
  } catch (_) {
    lead = null;
  }
...
exports.getDeliveryDetail = asyncHandler(async (req, res) => {
  ...
  return success(res, {
    delivery: mapDeliveryRow(delivery, lead, carrier, loadDetails),
  });
});
...
exports.contactDeliveryDriver = asyncHandler(async (req, res) => {
  ...
  if (!carrier || !carrier.phone)
    return notFound(res, "Driver contact not available yet");

  return success(res, {
    delivery: {
      title: delivery.loadDescription || delivery.description || "",
...
exports.sendDeliveryDriverSms = asyncHandler(async (req, res) => {
  ...
  const body =
    message?.trim() ||
    `Regarding delivery ${delivery.deliveryNumber} for ${lead.projectName}: please contact the customer at your earliest convenience.`;
...
exports.sendDeliveryCompanySms = asyncHandler(async (req, res) => {
  ...
  const carrier = await getDeliveryCarrier(delivery);
  if (!carrier || !carrier.phone)
    return notFound(res, "Delivery company contact not available yet");
...
exports.acknowledgeDeliveryReschedule = asyncHandler(async (req, res) => {
  ...
  if (!delivery.rescheduleHistory?.length)
    return badRequest(res, "This delivery has no reschedule to acknowledge");
...
exports.getDeliveryCalendarDetails = asyncHandler(async (req, res) => {
  ...
  if (!delivery.deliveryDate)
    return badRequest(res, "Delivery does not have a scheduled date yet");
...
exports.getDeliveryCalendar = asyncHandler(async (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="delivery-${delivery.deliveryNumber || delivery._id}.ics"`,
  );
  return res.send(ics);
});
...
exports.downloadDeliveryInfo = asyncHandler(async (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="delivery-${delivery.deliveryNumber || delivery._id}-details.pdf"`,
  );
...
exports.downloadDeliveryPackingList = asyncHandler(async (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="delivery-${delivery.deliveryNumber || delivery._id}-packing-list.pdf"`,
  );
  return res.send(buffer);
});
...
exports.downloadDeliveryInstructions = asyncHandler(async (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="delivery-${delivery.deliveryNumber || delivery._id}-instructions.pdf"`,
  );
  return res.send(buffer);
});
...
exports.getTaxReport = asyncHandler(async (req, res) => {
  ...
  const leads = await Lead.find({ customerId })
    .select("_id jobId projectName buildingType location")
    .lean();
  if (!leads.length)
    return success(res, {
      rows: [],
      ...
      page: parsedPage,
      limit: parsedLimit,
      summary: { totalContractAmount: 0, totalTaxDue: 0 },
    });

  const leadIds = leadId
    ? leads.filter((l) => String(l._id) === String(leadId)).map((l) => l._id)
    : leads.map((l) => l._id);
  const leadMap = new Map(leads.map((l) => [String(l._id), l]));
  ...
  const rows = invoices
    .filter(
      (inv) =>
        (inv.tax || 0) > 0 ||
        (inv.lineItems || []).some((li) => li.taxAmount > 0),
    )
    .map((inv) => {
      const lead = leadMap.get(String(inv.leadId)) || {};
      const lineTaxTotal = (inv.lineItems || []).reduce(
        (s, li) => s + (li.taxAmount || 0),
        0,
      );
      const taxDue = lineTaxTotal || inv.tax || 0;
      const taxRate =
        inv.totalAmount > 0 ? ((taxDue / inv.totalAmount) * 100).toFixed(2) : 0;
  ...
  const paged = rows.slice(
    ...
    (parsedPage - 1) * parsedLimit + parsedLimit,
  );
  ...
  return success(res, {
    ...
    page: parsedPage,
    limit: parsedLimit,
    summary: { totalContractAmount, totalTaxDue },
  });
});
...
const assertProjectOwner = async (req) => {
  const lead = await Lead.findById(req.params.leadId)
    .select("customerId projectName jobId location")
    .lean();
  if (!lead) return null;
...
exports.getProjectStats = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    Delivery.countDocuments({
      leadId,
      status: { $in: [...CUSTOMER_IN_TRANSIT_STATUSES] },
    }),
...
exports.getProjectTracking = asyncHandler(async (req, res) => {
  ...
  const [fullLead, tasks, milestones, stepDetails] = await Promise.all([
    Lead.findById(leadId)
      .select("projectName jobId endDate lifecycleStatus lifecycleHistory")
      .lean(),
  ...
  const timelineStatus =
    plannedCompletion && new Date(plannedCompletion) < now && completed < total
      ? "Delayed"
      : "On Track";
...
exports.getProjectPaymentsSummary = asyncHandler(async (req, res) => {
  ...
  const invoices = await Invoice.find({ leadId: req.params.leadId })
...
const fetchPlantBuildingDrawings = async (leadId, { buildingLabel } = {}) => {
  const targetNumber = buildingLabel
    ? buildingNumberFromLabel(buildingLabel)
    : null;
  ...
  for (const building of buildings) {
    if (targetNumber != null && building.buildingNumber !== targetNumber)
      continue;
    for (const drawing of building.drawings || []) {
...
const findPlantBuildingDrawing = async (leadId, docId) => {
  const buildings = await Building.find({ leadId }).select(
    "buildingNumber drawings",
  );
  for (const building of buildings) {
    const drawing = (building.drawings || []).find(
...
const buildingLabelsForLead = (lead) => {
  ...
  return Array.from(
    { length: count },
    (_, i) => `Building ${BUILDING_LETTERS[i] || i + 1}`,
  );
};
...
exports.getProjectDrawings = asyncHandler(async (req, res) => {
  ...
  const [drawings, plantDrawings, fullLead] = await Promise.all([
    ...
    Lead.findById(req.params.leadId)
      .select(
        "projectName jobId buildingType location lifecycleStatus numberOfBuildings documents",
      )
      .lean(),
  ]);

  const mergedDrawings = sortDrawingsNewestFirst([
    ...drawings,
    ...plantDrawings,
  ]);
...
exports.getAllProjectDrawings = asyncHandler(async (req, res) => {
  ...
  const [docs, plantBuildings] = await Promise.all([
    leadIds.length
      ? DrawingDocument.find({ leadId: { $in: leadIds } })
          .select("leadId category updatedAt")
          .lean()
      : Promise.resolve([]),
    leadIds.length
      ? Building.find({ leadId: { $in: leadIds } })
          .select("leadId buildingNumber drawings")
          .lean()
      : Promise.resolve([]),
  ]);
  ...
  const projects = leads.map((lead) => {
    ...
    const plantRows = plantBuildings.filter(
      (b) => String(b.leadId) === String(lead._id),
    );
    const plantDrawingCount = plantRows.reduce(
      (sum, b) => sum + (b.drawings || []).length,
      0,
    );
    ...
    const docLastUpdate = forLead.reduce(
      (max, d) => (!max || d.updatedAt > max ? d.updatedAt : max),
      null,
    );
    const lastUpdate =
      [docLastUpdate, plantLastUpdate]
        .filter(Boolean)
        .sort((a, b) => b - a)[0] || null;
    ...
    return {
      ...
      jobId: lead.jobId,
      location: lead.location,
      numberOfBuildings: Math.max(
        lead.numberOfBuildings || 1,
        plantRows.length,
      ),
      totalDrawings:
        forLead.filter((d) => d.category !== "document").length +
        plantDrawingCount,
      lastUpdate,
    };
  });
...
exports.getProjectBuildings = asyncHandler(async (req, res) => {
  const lead = await Lead.findById(req.params.leadId)
    .select("customerId projectName jobId location numberOfBuildings")
    .lean();
  if (!lead || String(lead.customerId) !== String(req.customer._id))
    return notFound(res, "Project not found");

  const [docs, plantBuildings] = await Promise.all([
    DrawingDocument.find({ leadId: req.params.leadId })
      .select("buildingLabel category updatedAt")
      .lean(),
  ...
  const labelsWithDocs = [
    ...new Set(docs.map((d) => d.buildingLabel || "Building A")),
  ];
  const plantLabels = plantBuildings.map((b) =>
    buildingLabelFromNumber(b.buildingNumber),
  );
  const allLabels = [
    ...new Set([
      ...buildingLabelsForLead(lead),
      ...labelsWithDocs,
      ...plantLabels,
    ]),
  ];
  ...
  const buildings = allLabels.map((label) => {
    const forBuilding = docs.filter(
      (d) => (d.buildingLabel || "Building A") === label,
    );
...
exports.getProjectMeetings = asyncHandler(async (req, res) => {
  ...
  const stats = {
    total,
    scheduled: await Meeting.countDocuments({
      leadId: req.params.leadId,
      status: "scheduled",
    }),
    completed: await Meeting.countDocuments({
      leadId: req.params.leadId,
      status: "completed",
    }),
    cancelled: await Meeting.countDocuments({
      leadId: req.params.leadId,
      status: "cancelled",
    }),
  };
...
exports.cancelProject = asyncHandler(async (req, res) => {
  const lead = await Lead.findById(req.params.leadId).select(
    "customerId isTerminated lifecycleStatus projectName jobId",
  );
  if (!lead) return notFound(res, "Project not found");
  if (String(lead.customerId) !== String(req.customer._id))
    return forbidden(res, "Not your project");
  if (lead.isTerminated) return badRequest(res, "Project is already cancelled");
  if (lead.lifecycleStatus === "delivered")
    return badRequest(res, "Cannot cancel a delivered project");
...
exports.getProjectRFQ = asyncHandler(async (req, res) => {
  const lead = await Lead.findById(req.params.leadId)
    .select(
      "customerId buildingType roofStyle sqft width length location description quoteValue isQuoteReady lifecycleStatus aiQuoteData aiContextSummary jobId projectName",
    )
    .lean();
  if (!lead) return notFound(res, "Project not found");
  if (String(lead.customerId) !== String(req.customer._id))
...
exports.updateProjectRFQ = asyncHandler(async (req, res) => {
  const lead = await Lead.findById(req.params.leadId).select(
    "customerId lifecycleStatus isTerminated",
  );
  if (!lead) return notFound(res, "Project not found");
  if (String(lead.customerId) !== String(req.customer._id))
    return forbidden(res, "Not your project");
  if (lead.isTerminated)
    return badRequest(res, "Cannot update a cancelled project");
  ...
  if (LOCKED_STAGES.includes(lead.lifecycleStatus)) {
    return badRequest(
      res,
      "Quote is already locked. Contact your sales rep to make changes.",
    );
  }
...
exports.approveProjectQuotation = asyncHandler(async (req, res) => {
  ...
  const quotation = await Quotation.findOne({ leadId: req.params.leadId }).sort(
    { createdAt: -1 },
  );
  if (!quotation) return notFound(res, "No quotation found for this project");
  if (quotation.status === "accepted")
    return badRequest(res, "Quotation is already accepted");
  ...
  return success(res, {
    ...
    quotation: { _id: quotation._id, status: quotation.status },
  });
});
...
exports.rejectProjectQuotation = asyncHandler(async (req, res) => {
  ...
  if (quotation.status === "rejected")
    return badRequest(res, "Quotation is already rejected");
...
exports.approveDrawing = asyncHandler(async (req, res) => {
  ...
  if (ref.source === "document") {
    ...
    if (doc.status === "approved")
      return badRequest(res, "Drawing is already approved");
    doc.status = "approved";
...
exports.requestDrawingRevision = asyncHandler(async (req, res) => {
  ...
  const { note } = req.body;
  if (!note || !note.trim())
    return badRequest(res, "Revision note is required");
...
exports.addDrawingComment = asyncHandler(async (req, res) => {
  ...
  if (global.io) {
    global.io
      .of("/admin")
      .to(`lead:${req.params.leadId}`)
      .emit("drawing_comment_added", {
...
const orderCountsForLead = async (leadId) => {
  const orders = await MaterialRequest.find({
    leadId,
  ...
  const quotations = await OrderQuotation.find({
  ...
  for (const order of orders) {
    const stage = computeOrderStage(
      order,
      latestQuotationByOrder.get(String(order._id)),
    );
...
exports.getProjectOrders = asyncHandler(async (req, res) => {
  ...
  const quotations = orders.length
    ? await OrderQuotation.find({ orderId: { $in: orders.map((o) => o._id) } })
        .sort({ createdAt: -1 })
        .lean()
    : [];
  const latestQuotationByOrder = new Map();
...
exports.createProjectOrder = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(items) || !items.length)
    return badRequest(res, "At least one order line item is required");

  for (const item of items) {
    if (!item.name || item.quantity == null)
      return badRequest(res, "Each item needs a coil type (name) and quantity");
  }
...
exports.getProjectOrderDetail = asyncHandler(async (req, res) => {
  ...
  const order = await MaterialRequest.findOne({
  ...
  const quotation = await OrderQuotation.findOne({ orderId: order._id })
    .sort({ createdAt: -1 })
    .lean();
  ...
  const deliveredItems = order.requestedItems.filter(
    (i) => i.deliveryStatus === "delivered",
  );
  const pendingItems = order.requestedItems.filter(
    (i) => i.deliveryStatus !== "delivered",
  );
  const totalQuantity = order.requestedItems.reduce(
    (sum, i) => sum + (i.quantity || 0),
...
exports.getQuotationsSummary = asyncHandler(async (req, res) => {
  ...
  const quotations = leadIds.length
    ? await OrderQuotation.find({ leadId: { $in: leadIds } })
        .select("leadId status sentAt")
        .lean()
    : [];

  const rows = leads.map((lead) => {
    const forLead = quotations.filter(
      (q) => String(q.leadId) === String(lead._id),
    );
    return {
      leadId: lead._id,
      projectName: lead.projectName,
      jobId: lead.jobId,
      location: lead.location,
      newQuotation: forLead.filter(
        (q) =>
          q.status === "sent" &&
          new Date(q.sentAt) >= new Date(Date.now() - 7 * 86400000),
      ).length,
...
exports.getOrderQuotationDetail = asyncHandler(async (req, res) => {
  const quotation = await OrderQuotation.findOne({
  ...
  const totalLength = quotation.lineItems.reduce(
    (s, i) => s + (i.lengthFeet || 0) * i.quantity,
    0,
  );
...
exports.approveOrderQuotation = asyncHandler(async (req, res) => {
  ...
  if (quotation.status !== "sent")
    return badRequest(res, `Quotation already ${quotation.status}`);

  quotation.status = "approved";
  quotation.respondedAt = new Date();
  if (req.body.customerRemark !== undefined)
    quotation.customerRemark = req.body.customerRemark;
  await quotation.save();

  await MaterialRequest.findByIdAndUpdate(quotation.orderId, {
    status: "approved",
  });

  await auditService.log({
    type: "lead",
    action: "order_quotation.approved",
  ...
  return success(res, {
    message: "Quotation Approved — Submitted Successfully",
    quotationId: quotation._id,
...
exports.rejectOrderQuotation = asyncHandler(async (req, res) => {
  ...
  await auditService.log({
    type: "lead",
    action: "order_quotation.rejected",
    leadId: quotation.leadId,
    customerId: req.customer._id,
    performedBy: req.customer._id,
    metadata: {
...
exports.cancelProjectOrder = asyncHandler(async (req, res) => {
  ...
  if (["fulfilled", "cancelled"].includes(order.status))
    return badRequest(res, `Order already ${order.status}`);
...
exports.getChatMessages = asyncHandler(async (req, res) => {
  ...
  if (!leadId) return badRequest(res, "leadId is required");
  if (!CHAT_CHANNELS.some((c) => c.key === channel))
    return badRequest(res, "Invalid channel");
  ...
  await Message.updateMany(
    { leadId, channel, senderType: { $ne: "customer" }, isRead: false },
    { isRead: true },
  );
...
exports.sendChatMessage = asyncHandler(async (req, res) => {
  ...
  if (!leadId || !content?.trim())
    return badRequest(res, "leadId and content are required");
  if (!CHAT_CHANNELS.some((c) => c.key === channel))
    return badRequest(res, "Invalid channel");
...
exports.getCustomerNotifications = asyncHandler(async (req, res) => {
  ...
  else if (NOTIFICATION_CATEGORY_MAP[filter])
    baseFilter.type = { $in: NOTIFICATION_CATEGORY_MAP[filter] };
  ...
  const stats = {
    ...
    highPriority: await Notification.countDocuments({
      customerId,
      priority: "high",
    }),
...
exports.markCustomerNotificationRead = asyncHandler(async (req, res) => {
  ...
  return success(
    res,
    { notificationId: notification._id, isRead: true },
...
exports.markAllCustomerNotificationsRead = asyncHandler(async (req, res) => {
  await Notification.updateMany(
    { customerId: req.customer._id, isRead: false },
    { isRead: true },
  );
...
exports.confirmDeliverySiteReady = asyncHandler(async (req, res) => {
  ...
  return success(
    res,
    { deliveryId: full._id, siteReadyConfirmation: full.siteReadyConfirmation },
    "Site ready confirmed",
  );
});
...
exports.confirmDeliveryEquipment = asyncHandler(async (req, res) => {
  ...
  const {
    forkliftAvailable,
    craneOrHeavyMachineryAvailable,
    storageAreaReady,
    toolsAndAccessoriesOnSite,
  } = req.body;
  ...
  return success(
    res,
    { deliveryId: full._id, equipmentConfirmation: full.equipmentConfirmation },
    "Equipment confirmed",
  );
});
...
const assertBundleOwner = async (req, bundleId) => {
  const bundle = await Bundle.findOne({
    $or: [
      { _id: bundleId.length === 24 ? bundleId : null },
      { bundleNo: bundleId },
    ],
  })
    .populate(
      "packingListId",
      "packingListNo truckLabel truckType deliveryLocation",
    )
    .lean();
  if (!bundle) return null;
  const lead = await Lead.findById(bundle.leadId)
    .select("customerId projectName jobId location")
    .lean();
  if (!lead || String(lead.customerId) !== String(req.customer._id))
    return null;
...
const buildBundleScanCard = async ({ bundle, lead }) => {
  ...
  return {
    ...
    truck: bundle.packingListId
      ? {
          packingListNo: bundle.packingListId.packingListNo,
          truck:
            bundle.packingListId.truckLabel || bundle.packingListId.truckType,
        }
      : null,
    deliveryReference: delivery
      ? {
          deliveryNumber: delivery.deliveryNumber,
          destination: delivery.deliveryLocation || lead.location,
          status: delivery.status,
        }
      : null,
  };
...
exports.reportCustomerBundleIssue = asyncHandler(async (req, res) => {
  ...
  await Bundle.findByIdAndUpdate(owned.bundle._id, {
    mismatchNotes: issue.trim(),
    mismatchReportedAt: new Date(),
  });

  await auditService.log({
    type: "lead",
    action: "bundle.issue_reported",
    leadId: owned.lead._id,
    customerId: req.customer._id,
    performedBy: req.customer._id,
    metadata: {
      bundleId: owned.bundle._id,
      bundleNo: owned.bundle.bundleNo,
      issue: issue.trim(),
    },
  });
...
exports.contactSupportForBundle = asyncHandler(async (req, res) => {
  ...
  const fullLead = await Lead.findById(owned.lead._id)
    .select("assignedSales")
    .populate("assignedSales", "name email")
    .lean();
...
exports.downloadBundleContents = asyncHandler(async (req, res) => {
  ...
  const buffer = await generatePackingListDetailPdf(
    {
      packingListNo: owned.bundle.bundleNo,
      truck: "",
      destination: owned.lead.location,
      totalBundles: 1,
      totalWeight: owned.bundle.totalWeight,
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="bundle-${owned.bundle.bundleNo}-contents.pdf"`,
  );
  return res.send(buffer);
});
...
exports.downloadBundlePackingList = asyncHandler(async (req, res) => {
  ...
  const siblingBundles = owned.bundle.packingListId
    ? await Bundle.find({ packingListId: owned.bundle.packingListId._id })
        .select("bundleNo bundleType totalQty totalWeight status")
        .lean()
    : [owned.bundle];
  ...
  const buffer = await generatePackingListDetailPdf(
    {
      packingListNo:
        owned.bundle.packingListId?.packingListNo || owned.bundle.bundleNo,
      truck:
        owned.bundle.packingListId?.truckLabel ||
        owned.bundle.packingListId?.truckType ||
        "",
      destination:
        owned.bundle.packingListId?.deliveryLocation || owned.lead.location,
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="bundle-${owned.bundle.bundleNo}-packing-list.pdf"`,
  );
  return res.send(buffer);
});
