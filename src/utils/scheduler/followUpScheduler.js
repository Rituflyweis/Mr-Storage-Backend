...
const MANUAL_SOURCES = new Set(['manual', '', null, undefined])
...
const isManualFollowUp = (followUp) => MANUAL_SOURCES.has(followUp?.source)
...
const scheduleFollowUpReminder = (followUp) => {
  ...
  if (!isManualFollowUp(followUp)) return
  ...
  // Strict T-minus reminder only. If 30 minutes before is already past, do not emit.
  if (jobDate.getTime() <= Date.now()) return

  schedule.scheduleJob(jobName, jobDate, () => {
