  ...
  if (wCol >= 0) {
    ...
    for (let r = hRow + 1; r < data.length; r++) {
      const rs = data[r]
        ...
        .join(" ")
        .toLowerCase();
      if (rs.includes("total") || rs.includes("recd") || rs.includes("unless"))
        continue;
      const v = parseFloat(String(data[r][wCol] || "").replace(/,/g, ""));
...
const getTabCat = (name, customTabRules = []) => {
  ...
  for (const r of customTabRules) {
    if (
      r.matchType === "tab_name" &&
      r.match &&
      n.includes(String(r.match).toLowerCase().trim())
    ) {
      return r.cat || "trim";
    }
  }
  ...
  if (
    n === "sheeting" ||
    n.includes("sheeting") ||
    n.includes("wall sheeting") ||
    n.includes("roof sheeting")
  ) {
    return "sheeting";
  }
  if (
...
const applyItemRules = (data, sheetName, sf, customTabRules = []) => {
  ...
  for (let r = hRow + 1; r < data.length; r++) {
    ...
    if (!qty) continue;
    const part = String(row[cols.part] || "")
      .trim()
      .toLowerCase();
    const desc = String(row[cols.desc] || "")
      .trim()
      .toLowerCase();
    const markVal =
      cols.mark !== undefined
        ? String(row[cols.mark] || "")
            .trim()
            .toLowerCase()
        : "";
    ...
    for (const cr of customTabRules) {
      ...
      let hit = false;
      if (
        cr.matchType === "part_number" &&
        (part.includes(matchStr) || markVal.includes(matchStr))
      )
        hit = true;
      ...
      if (price > 0) {
        matched.push({
          label:
            cr.note ||
            (desc ? desc.charAt(0).toUpperCase() + desc.slice(1) : cr.match),
          cat: cr.cat || "trim",
          method,
          qty,
          lf,
...
const emptyCategories = () => ({
  ...
  secondary: {
    label: "Purlins, Girts & Eave Struts",
    weight: 0,
    tag: "cat-secondary",
  },
