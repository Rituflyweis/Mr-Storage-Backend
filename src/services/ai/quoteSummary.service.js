  ...
  try {
    const details = [
      ...
      quotation.includedMaterials?.length
        ? `Materials: ${quotation.includedMaterials.map((m) => m.name).join(", ")}`
        : "",
      quotation.optionalAddOns?.length
        ? `Add-ons: ${quotation.optionalAddOns.map((a) => a.name).join(", ")}`
        : "",
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 512,
      system:
        "You write brief plain-text summaries of construction quotations for a CRM. Be factual, concise, and clear. No markdown. Anyone reading this should understand the quote in 30 seconds.",
      messages: [
        {
          role: "user",
