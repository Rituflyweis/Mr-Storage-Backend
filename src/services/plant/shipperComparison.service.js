...
const SERVICE_VERSION =
  "shipper-comparison-v4.8-row-preserved-no-length-normalization";
...
const fractionishToNumber = (value) => {
  ...
  if (mixedHyphen) {
    return (
      Number(mixedHyphen[1]) + Number(mixedHyphen[2]) / Number(mixedHyphen[3])
    );
  }
  ...
  if (mixedSpace) {
    return (
      Number(mixedSpace[1]) + Number(mixedSpace[2]) / Number(mixedSpace[3])
    );
  }
...
const sameNoLength = (aFeet, bFeet) =>
  isNoLengthValue(aFeet) && isNoLengthValue(bFeet);
...
const normalizeColor = (value) => {
  ...
  if (/RED\s*OXIDE|\bRO\b|\bR\.O\.\b|\bPRIME\s*,?\s*R\b|OXIDE/.test(s))
    return "RED_OXIDE";
  if (/GALV|GALVANIZED/.test(s)) return "GALVANIZED";
  if (/\bWH\b|WHITE/.test(s)) return "WHITE";
  if (/\bBLK\b|BLACK/.test(s)) return "BLACK";
...
const normalizeShape = (value) => {
  const s = String(value || "").toUpperCase();
  if (
    /\bCEE\b|\bCEES\b|\bC\s*PURLIN\b|\bC-PURLIN\b|\bPURLIN\b.*\bCEE\b/.test(s)
  )
    return "CEE";
  if (/\bZEE\b|\bZEES\b|\bZ\s*PURLIN\b|\bZ-PURLIN\b/.test(s)) return "ZEE";
  if (/CHANNEL/.test(s)) return "CHANNEL";
...
const normalizeMaterial = ({ partCode, description, color, partColor }) => {
  ...
  const rawDescription = cleanStr(description);
  const combined =
    `${rawPartCode} ${rawDescription} ${color || ""} ${partColor || ""}`.toUpperCase();
  ...
  let flangeInches = null;
  let normalizedColor =
    normalizeColor(color || partColor) ||
    normalizeColor(rawDescription) ||
    normalizeColor(rawPartCode);
  const warnings = [];
  ...
  if (central) {
    ...
    flangeInches = Number(central[3]) / 10;
    gauge = Number(central[4]);
    if (!normalizedColor && central[5].includes("R"))
      normalizedColor = "RED_OXIDE";
  }

  // Quicken style: PC16-RO-8X3.5, PC12-RO-8X2.5
  const quicken = code.match(
    /^(P?[CZ])?(\d{2})-?([A-Z]{1,3})?-?(\d+(?:\.\d+)?)X(\d+(?:\.\d+)?)/,
  );
  if (quicken) {
    if (!shape)
      shape = code.startsWith("PZ") || code.startsWith("Z") ? "ZEE" : "CEE";
    gauge = Number(quicken[2]);
    if (!normalizedColor && quicken[3])
      normalizedColor = normalizeColor(quicken[3]);
    depthInches = Number(quicken[4]);
    flangeInches = Number(quicken[5]);
  }
  ...
  if (!depthInches || !flangeInches) {
    const dim = combined.match(
      /(\d+(?:\.\d+)?)\s*"?\s*X\s*(\d+(?:\.\d+)?|\d+\s*-\s*\d+\s*\/\s*\d+|\d+\s+\d+\s*\/\s*\d+)\s*"?/,
    );
    if (dim) {
      depthInches = depthInches || Number(dim[1]);
      flangeInches = flangeInches || parseDimensionValue(dim[2]);
  ...
  const materialKey = buildMaterialKey({
    shape,
    gauge,
    depthInches,
    flangeInches,
  ...
  if (materialKey) confidence = 0.95;
    ...
    warnings.push(
      "Unable to build canonical material key from part code/description",
    );
...
const materialCompatibility = (expectedMaterial, receivedMaterial) => {
  ...
  if (e.materialKey && r.materialKey && e.materialKey === r.materialKey) {
    return {
      compatible: true,
      confidence: 0.99,
      reason: "Canonical material key matched.",
    };
  }

  const essentialFields = ["shape", "gauge", "depthInches", "flangeInches"];
  const bothHaveEssentials = essentialFields.every(
    (k) => e[k] != null && r[k] != null,
  );
  if (bothHaveEssentials) {
    const sameEssentials =
    ...
    if (sameEssentials && colorCompatible) {
      return {
        compatible: true,
        confidence: 0.93,
        reason: "Canonical material dimensions matched.",
      };
    }
  ...
  return {
    ...
    confidence: 0.55,
    reason:
      "Material could not be fully verified from available part code/description.",
  };
};
...
const detectVendorFormat = (text) => {
  ...
  if (
    ...
    (t.includes("PIECE MARK:") && t.includes("SALES ORDER"))
  ) {
    return "quicken_steel";
  }
  return "generic_material_pdf";
...
const parseCentralStates = (text) => {
  ...
  for (const raw of lines) {
    ...
    // totals row can appear after Part Mark in extracted text.
    const totals = line.match(
      /^([\d,]+(?:\.\d+)?)\s+(LF|FT|EA|LB)\s+([\d.]+)\s+([\d,]+(?:\.\d{2})?)$/i,
    );
    if (totals && cur) {
      cur.totalLinearFeet = Number(totals[1].replace(/,/g, ""));
      cur.uom =
        totals[2].toUpperCase() === "FT" ? "LF" : totals[2].toUpperCase();
      cur.unitPrice = Number(totals[3]);
      cur.amount = Number(totals[4].replace(/,/g, ""));
      cur.rawParts.push(line);
      continue;
    ...
    // ("RF Column") does not.
    const head =
      !/^\d+\s+Pieces?\b/i.test(line) && line.match(/^(\d+)\s+(.+)$/);
    if (head) {
      flush();
      const rest = head[2].trim();
      const firstTok = rest.split(/\s+/)[0];
      ...
      const looksLikeAlphaProduct =
        ...
        restAfterFirstToken.length > 0;
      const knownDescriptionLead =
        /^(RF|EW|EXT|WIND|DOOR|ROOF|WALL|RIGID|ENDWALL|ANCHOR|BOLTS?|WASHERS?|FASTENERS?|ACCESSORIES?|TRIM)$/i.test(
          firstTok,
        );
      const looksLikeProduct =
        looksLikeNumericProduct ||
        (looksLikeAlphaProduct && !knownDescriptionLead);
      cur = {
        lineNo: head[1],
        product: looksLikeProduct ? firstTok : null,
        description: looksLikeProduct ? restAfterFirstToken : rest,
...
const parseQuicken = (text) => {
  ...
  for (const raw of lines) {
    ...
    const pageM =
      line.match(/Page\s+Number:\s*(\d+)/i) ||
      line.match(/Page\s+(\d+)\s+of\s+\d+/i);
    if (pageM) pageNumber = Number(pageM[1]);

    if (
      /^(Line\s+Item\s+Description|QUICKEN STEEL|SOLD TO|SHIP TO|Print Date|Document Ref)/i.test(
        line,
      )
    )
      continue;
...
${String(text || "").slice(0, 120000)}`;
  ...
  const response = await client.messages.create({
    ...
    temperature: 0,
    system:
      "You extract steel vendor quote material rows into strict JSON. You never compare, summarize, or invent values.",
    messages: [{ role: "user", content: prompt }],
  });
...
const parseTabular = (rows) => {
  ...
  const headerIdx = rows.findIndex((row) => {
    ...
    const hasMark =
      n.includes("MARK") ||
      n.includes("PIECEMARK") ||
      n.includes("PARTMARK") ||
      n.includes("MARKID");
    const hasDesc =
      n.includes("DESCRIPTION") ||
      n.includes("DESC") ||
      n.includes("ITEMDESCRIPTION");
    return hasQty && (hasMark || hasDesc);
  });
  if (headerIdx < 0) return [];
  ...
  if (ext === "pdf") {
    ...
    if (fmt === "central_states") {
      ...
      if (parsed.items.length)
        return { ...parsed, extractionMethod: "pdf_text" };
    }
 ...
 * ========================================================= */
const toVendorQuoteLineDoc = (item, request, format, extractionMethod) => {
  const cleanedDescriptionForMaterial = cleanMaterialDescription(
    item.description,
  );
  const material = normalizeMaterial({
    partCode: item.product,
  ...
  return {
    ...
    unitPrice: item.unitPrice ?? null,
    priceUnit: ["EA", "FT", "LF", "LB", "LOT"].includes(
      String(item.priceUnit || "").toUpperCase(),
    )
      ? String(item.priceUnit).toUpperCase()
      : "UNKNOWN",
...
const splitMarkList = (value) => {
  ...
  const s = cleanPartMarkText(value);
  if (!s || ["-", "NA", "N/A", "NULL", "NONE"].includes(s.toUpperCase()))
    return [];
...
const normalizeExpectedFromBomItemsForShipper = (bomItems) => {
  ...
  for (const item of bomItems || []) {
    ...
    const colorKey =
      item.partColorNormalized || normCode(item.partColor) || "_";
    const lengthKey =
      parsedLengthFeet != null && Number.isFinite(Number(parsedLengthFeet))
        ? Number(parsedLengthFeet).toFixed(4)
        : "_";
...
const buildExpectedRowsForComparison = async (
  consolidatedBOM,
  options = {},
) => {
  const bomItemIds = uniq(
    ...
    if (bomItems.length) {
      ...
      return {
        rows: rowPreserved
          ...
          : normalizeExpectedFromBomItemsForShipper(bomItems),
        meta: {
          expectedSource: rowPreserved
            ? "bom_items_row_preserved"
            : "bom_items_shipper_group_row_audited",
          bomItemIdsFound: bomItemIds.length,
          bomItemsLoaded: bomItems.length,
        },
      };
...
const groupSide = (rows, { mark, len, qty, id, materialGetter }) => {
  ...
  for (const r of rows) {
    ...
    if (r.description) g.descriptions.add(r.description);
    if (r.material) g.materials.push(r.material);
    if (r.rawRow?.canonicalMaterial)
      g.materials.push(r.rawRow.canonicalMaterial);
    if (r.qtyShared) g.qtyShared = true;
    g.sourceLineCount += 1;
  }
...
const groupSideRowPreserved = (
  rows,
  { mark, len, qty, id, materialGetter },
) => {
  const map = new Map();
  const occurrenceByBaseKey = new Map();
  ...
  for (const r of rows || []) {
    ...
    if (r.material) group.materials.push(r.material);
    if (r.rawRow?.canonicalMaterial)
      group.materials.push(r.rawRow.canonicalMaterial);
...
const anonymousRowCompatible = (exp, rec) => {
  ...
  if (
    expDesc &&
    recDesc &&
    (expDesc.includes(recDesc) || recDesc.includes(expDesc))
  ) {
    return true;
  }
  ...
  // description starts with that code, treat it as the same anonymous material.
  if (expPartCodes.length && recDesc) {
    return expPartCodes.some(
      (code) => recDesc.startsWith(code) || recDesc.includes(code),
    );
  }
  if (recPartCodes.length && expDesc) {
    return recPartCodes.some(
      (code) => expDesc.startsWith(code) || expDesc.includes(code),
    );
  }
...
const scoreRowPreservedCandidate = (exp, rec) => {
  ...
  const qtyDiff = Math.abs(
    Number(rec.totalQty || 0) - Number(exp.totalQty || 0),
  );
  score += qtyDiff * 100000;
  ...
  if (expDesc && recDesc) {
    if (expDesc === recDesc) score -= 250;
    else if (expDesc.includes(recDesc) || recDesc.includes(expDesc))
      score -= 100;
  }
  ...
  // Occurrence order is only a tie-breaker, never the primary matching signal.
  score +=
    Math.abs(Number(exp.rowOccurrence || 0) - Number(rec.rowOccurrence || 0)) /
    1000;
  ...
  if (exp.baseKey) {
    ...
    if (sameBaseCandidates.length) {
      const ranked = sameBaseCandidates
        .map((group) => ({
          group,
          score: scoreRowPreservedCandidate(exp, group),
        }))
        .sort((a, b) => a.score - b.score);

      return {
        type:
          ranked[0].group.key === exp.key
            ? "exact"
            : "row_preserved_best_occurrence",
        group: ranked[0].group,
        duplicatePairingScore: ranked[0].score,
      };
    }
...
const findReceivedMatch = (exp, receivedMap, used) => {
  ...
  const sameMark = [...receivedMap.values()].filter(
    (r) =>
      !used.has(r.key) &&
      r.markNormalized &&
      r.markNormalized === exp.markNormalized,
  );

  if (!sameMark.length) {
    // Frame/no-part-code fallback:
    // match only when RF/EW mark canonical forms are equal and length is within tolerance.
    const expHasPartCode = Boolean(
      exp.samplePartCode && normCode(exp.samplePartCode),
    );
    const expFrameKey = !expHasPartCode ? frameMarkKey(exp.mark) : null;
    ...
    if (expFrameKey) {
      ...
      const frameWithinTolerance = frameCandidates
        .map((r) => ({
          group: r,
          diff: lengthDiffInches(exp.lengthFeet, r.lengthFeet),
        }))
        .filter((x) => x.diff != null && x.diff <= LENGTH_TOLERANCE_INCH)
        .sort((a, b) => a.diff - b.diff);
    ...
    const anonymousCandidates = [...receivedMap.values()]
      .filter((r) => !used.has(r.key) && anonymousRowCompatible(exp, r))
      .sort((a, b) => {
        const aPartHit = [...(exp.partCodes || [])].some((code) =>
          (a.partCodes || new Set()).has(code),
        )
          ? 0
          : 1;
        const bPartHit = [...(exp.partCodes || [])].some((code) =>
          (b.partCodes || new Set()).has(code),
        )
          ? 0
          : 1;
        return aPartHit - bPartHit;
  ...
  if (withinTolerance.length) {
    return {
      ...
      group: withinTolerance[0].group,
      diffInches: withinTolerance[0].diff,
    };
  }
  ...
  return {
    type: "same_mark_length_mismatch",
    group: nearest?.group || sameMark[0],
    diffInches: nearest?.diff ?? null,
  };
};
...
const compareMaterials = (
  ...
  const enforceRowPreservedAudit = Boolean(options.enforceRowPreservedAudit);
  const sideGrouper = enforceRowPreservedAudit
    ? groupSideRowPreserved
    : groupSide;
  ...
  const expectedInputLineCount = Array.isArray(expectedRows)
    ? expectedRows.length
    : expected.size;
  const vendorComparisonLineCount = received.size;
  ...
  for (const exp of expected.values()) {
      ...
      results.push(
        buildResult(request, {
          ...
          difference: {
            lengthDiffFeet: round(safe(rec.lengthFeet) - safe(exp.lengthFeet)),
            lengthDiffInches:
              match.diffInches == null ? null : round(match.diffInches, 3),
          },
        }),
      );
      exceptions.push({
    ...
    const materialNote =
      matCheck.compatible === true ? matCheck.reason : matCheck.reason;
    const confidence = matCheck.compatible === true ? 0.99 : 0.92;
    const reason = `Matched on piece mark, length and quantity. ${materialNote}`;
  ...
  if (enforceRowPreservedAudit && expectedInputLineCount > expected.size) {
    ...
    for (const r of results) {
      ...
      const hiddenExpectedRows = Math.max(
        0,
        Number(r.expected?.sourceLineCount || 1) - 1,
      );
      if (!hiddenExpectedRows) continue;
        ...
        auditRows.push(
          buildResult(request, {
            ...
            expected: {
              ...(r.expected || {}),
              rowPreservedAudit: true,
              collapsedComparisonSourceLineCount:
                r.expected?.sourceLineCount || null,
            },
            received: null,
            difference: {
              qtyDiff: 0,
  ...
  const summary = {
    expectedLines: enforceRowPreservedAudit
      ? expectedInputLineCount
      : expected.size,
    vendorLines: vendorComparisonLineCount,
    matchedLines: count("matched"),
    missingItems: count("missing_in_vendor_quote"),
...
const compareShipperRequest = async (requestId) => {
  ...
  try {
    ...
    if (!vendorItems.length) {
      throw new Error(
        "No vendor line items could be extracted from the submitted file",
      );
    }
    ...
    const vendorDocs = await VendorQuoteLine.insertMany(
      vendorItems.map((it) =>
        toVendorQuoteLineDoc(it, request, format, extractionMethod),
      ),
      { ordered: false },
    );
    ...
    const rowPreservedConsolidated =
      consolidatedItems.length > 0 &&
      consolidatedItems.every(
        (item) =>
          Number(item.sourceLineCount || 1) === 1 &&
          (item.bomItemIds || []).length <= 1,
      );

    const { rows: expectedRows, meta: expectedMeta } =
      await buildExpectedRowsForComparison(consolidatedBOM, {
        rowPreserved: rowPreservedConsolidated,
      });

    const receivedRows = expandReceivedRows(
      vendorDocs.map((d) => ({
        _id: d._id,
        pieceMark: d.pieceMark,
        lengthFeet: d.lengthFeet,
    ...
    const { results, summary, exceptions } = compareMaterials(
      expectedRows,
      receivedRows,
      request,
      {
