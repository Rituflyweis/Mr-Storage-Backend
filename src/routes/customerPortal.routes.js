...
router.post(
  "/projects/:leadId/quotation/approve",
  ctrl.approveProjectQuotation,
);
router.post("/projects/:leadId/quotation/reject", ctrl.rejectProjectQuotation);
router.get(
  "/projects/:leadId/payments/summary",
  ctrl.getProjectPaymentsSummary,
);
...
router.get(
  "/projects/:leadId/buildings/:buildingLabel",
  ctrl.getBuildingDrawings,
);
router.post("/projects/:leadId/drawings/:docId/approve", ctrl.approveDrawing);
router.post(
  "/projects/:leadId/drawings/:docId/request-revision",
  ctrl.requestDrawingRevision,
);
router.post(
  "/projects/:leadId/drawings/:docId/comments",
  [body("text").trim().notEmpty()],
  validate,
  ctrl.addDrawingComment,
);
...
router.post(
  "/projects/:leadId/orders/:orderId/cancel",
  ctrl.cancelProjectOrder,
);
router.get(
  "/projects/:leadId/order-quotations",
  ctrl.getProjectOrderQuotations,
);
...
router.post(
  ...
  [
    ...
    body("expectedStartDate")
      .notEmpty()
      .withMessage("expectedStartDate is required")
      .isISO8601(),
    body("targetCompletionDate")
      .notEmpty()
      .withMessage("targetCompletionDate is required")
      .isISO8601(),
    body("fullAddress")
      .if(body("location").not().exists())
      .notEmpty()
...
router.post(
  "/payments/invoices/:invoiceId/payment-proof",
...
router.get(
  "/deliveries/:deliveryId/calendar/details",
  ctrl.getDeliveryCalendarDetails,
);
router.get("/deliveries/:deliveryId/download", ctrl.downloadDeliveryInfo);
router.get(
  "/deliveries/:deliveryId/download/packing-list",
  ctrl.downloadDeliveryPackingList,
);
router.get(
  "/deliveries/:deliveryId/download/instructions",
  ctrl.downloadDeliveryInstructions,
);
router.post(
  "/deliveries/:deliveryId/contact-driver",
  ctrl.contactDeliveryDriver,
);
router.post(
  "/deliveries/:deliveryId/contact-driver/sms",
  ctrl.sendDeliveryDriverSms,
);
router.post(
  "/deliveries/:deliveryId/contact-company",
  ctrl.contactDeliveryCompany,
);
router.post(
  "/deliveries/:deliveryId/contact-company/sms",
  ctrl.sendDeliveryCompanySms,
);
router.post(
  "/deliveries/:deliveryId/confirmation-email",
  ctrl.sendDeliveryConfirmation,
);
router.post(
  "/deliveries/:deliveryId/request-callback",
  ctrl.requestDeliveryCallback,
);
router.post(
  "/deliveries/:deliveryId/acknowledge-reschedule",
  ctrl.acknowledgeDeliveryReschedule,
);
router.get("/deliveries/:deliveryId/documents", ctrl.getDeliveryDocuments);
router.post(
  "/deliveries/:deliveryId/confirm-site-ready",
  ctrl.confirmDeliverySiteReady,
);
router.post(
  "/deliveries/:deliveryId/confirm-equipment",
  ctrl.confirmDeliveryEquipment,
);
...
router.post(
  "/order-quotations/:quotationId/approve",
  ctrl.approveOrderQuotation,
);
...
router.get(
  "/bundles/:bundleId/download/packing-list",
  ctrl.downloadBundlePackingList,
);
...
router.get(
  ...
  ctrl.getCustomerUnreadNotificationCount,
);
